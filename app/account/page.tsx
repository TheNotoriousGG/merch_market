...
import { loadAccount, logoutCustomer, startPhoneAuthentication, verifyPhone, type CustomerAccount, type PhoneChallenge } from "./account-api";
...
export default function AccountPage() {
  ...
  return <main className="utility-page account-page">
    ...
    <div className="utility-shell">
      ...
      {loading?<section className="account-auth-card account-auth-loading"><span>Загружаем профиль…</span></section>:account?<section className="account-grid"><section className="account-profile-card"><span className="account-avatar">А</span><div><span className="section-kicker">Ваш профиль</span><h2>{account.displayName||"Покупатель Amra"}</h2><p>{account.phone}</p></div><button disabled={busy} onClick={()=>void logout()}>Выйти</button></section><section className="account-address-card"><span className="section-kicker">Доставка</span><h2>Адреса</h2><p>Сохранённые адреса появятся здесь после первого заказа.</p></section><section className="account-bonus-card"><span className="section-kicker">Заказы</span><strong>0</strong><p>История заказов</p><i>Всё впереди</i></section></section>:<section className="account-auth-layout">
