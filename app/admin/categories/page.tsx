...
export default function Categories() {
  ...
  return <>
    ...
    <section className={styles.panel}><div className={styles.panelHeading}><h2>{showArchive?"Архив категорий":"Категории каталога"}</h2><button className={styles.secondary} onClick={()=>setShowArchive(value=>!value)}>{showArchive?"Вернуться к категориям":`Архив · ${items.filter(item=>item.status==="ARCHIVED").length}`}</button></div>{visibleItems.map((item) => <div className={styles.categoryRow} key={item.id}>
