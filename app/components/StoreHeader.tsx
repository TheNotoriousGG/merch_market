  ...
  return <header className={`site-header ${isScrolled ? "is-scrolled" : ""}`}>
    ...
    <div className="header-center">
      ...
      <div className="header-actions">
        <button className="header-icon search-icon" aria-label="Поиск" title="Поиск">
          <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="7" /><path d="m20 20-4-4" /></svg>
        </button>
        <Link className="header-icon heart-icon" href="/favorites" aria-label={`Избранное, ${favoriteCount} ${productWord(favoriteCount)}`} title="Избранное">
        ...
        <Link className="header-icon account-icon" href="/account" aria-label="Личный кабинет" title="Личный кабинет">
          <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="4" /><path d="M4.5 21a7.5 7.5 0 0 1 15 0" /></svg>
        </Link>
        <Link className="header-icon bag-icon" href="/cart" aria-label={`Корзина, ${cartCount} ${productWord(cartCount)}`} title="Корзина">
