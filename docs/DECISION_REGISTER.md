...
Практический маршрут по этим решениям, runtime chains и extension workflow находится в `docs/DEVELOPER_GUIDE.md`; guide объясняет принятые решения, но не заменяет ADR и этот реестр.
...
- Inventory warehouse audit хранит только allowlisted quantity/version diff и request correlation в той же транзакции; representative index/timeout acceptance зафиксирован в `docs/persistence/INVENTORY_QUERY_PLANS.md`.
