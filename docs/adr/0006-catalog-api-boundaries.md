...
- Administrative create commands use idempotency keys. Mutating commands use CSRF protection; commands against existing aggregates additionally require `If-Match` and return `428` when absent or `412` when stale.
