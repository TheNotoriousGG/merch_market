...
export default function CatalogProductDialog({ product, onClose }: { product: CatalogProduct; onClose: () => void }) {
  ...
  const purchasable = product.priceAvailable !== false;
  ...
  return <div className="product-dialog-backdrop" role="button" tabIndex={-1} aria-label="Закрыть карточку" onKeyDown={(event) => event.key === "Escape" && onClose()} onClick={(event) => event.target === event.currentTarget && onClose()}>
    <section className="product-dialog" role="dialog" aria-modal="true" aria-labelledby="product-dialog-title">
      ...
      <div className={`product-dialog-visual ${product.colorClass}`}><span className="new-badge">{product.isNew ? "NEW" : "AMRA"}</span>{product.imageUrl ? <img className="catalog-product-photo" src={product.imageUrl} alt="" /> : <span className={`product-object product-object-${product.art}`} aria-hidden="true" />}</div>
      <div className="product-dialog-copy">
        ...
        <strong className="product-dialog-price">{purchasable ? `${product.price.toLocaleString("ru-RU")} ₽` : "Цена появится позже"}</strong>
        ...
        <div className="product-dialog-actions"><button disabled={!purchasable || (!selectedSize && cartQuantity === 0)} className={`cart-action-button ${cartQuantity > 0 ? "is-added" : ""}`} onClick={handleCartAction}><CartButtonContent added={cartQuantity > 0} label={!purchasable ? "Скоро в продаже" : cartQuantity > 0 ? "В корзине · Перейти" : selectedSize ? "В корзину" : "Выберите размер"} /></button><button className={liked ? "liked" : ""} onClick={() => toggleFavorite(toShopProduct(product))} aria-pressed={liked} aria-label="Добавить в избранное"><FavoriteIcon active={liked} /></button></div>
