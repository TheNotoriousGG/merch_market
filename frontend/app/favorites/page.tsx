...
export default function FavoritesPage() {
  ...
  const { favorites, toggleFavorite, isInCart } = useShop();
    ...
    <div className="utility-shell">
      ...
      {favorites.length === 0 ? <section className="utility-empty">
        ...
        {favorites.map((product) => <article className="favorite-card" key={product.id}>
          ...
          <div className="favorite-copy"><div><button className="favorite-title-button" onClick={() => setSelectedProduct(product)}><h2>{product.name}</h2></button><strong>{formatPrice(product.price)}</strong></div><button className={`cart-action-button ${isInCart(product.id)?"is-added":""}`} onClick={() => isInCart(product.id)?router.push("/cart"):setSelectedProduct(product)}><CartButtonContent added={isInCart(product.id)} label={isInCart(product.id)?"В корзине · Перейти":"Выбрать вариант"} /></button></div>
        </article>)}
      </section>}
    </div>
